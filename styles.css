(() => {
  ...
  const FINAL_IMAGE_URL = "https://i.pinimg.com/1200x/eb/59/fa/eb59fa1edfc4dbb550313c4c046bcdeb.jpg";
  ...
  function showFinal(){
    // preload-ish (browser caches) – img already in DOM, but ensure src
    const img = $("finalImg");
  ...
  function start(){
    ...
    // faster effect at start, then “calms” a bit
    function frame(){
      if(!running) return;

      const elapsed = Date.now() - startAt;
      const t = clamp(elapsed / DURATION_MS, 0, 1);
      const intensity = 1 - (t * 0.25); // slight dampening
